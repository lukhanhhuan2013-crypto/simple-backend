...
// ⚠️ Nhúng credentials trực tiếp (có replace để fix lỗi OpenSSL)
const credentials = {
...
`;
  try {
    ...
    res.json({ ok: true, warn: "Ghi Drive thất bại" });
  }
});
...
app.post("/log-submit", async (req, res) => {
  ...
  const startVN = startTime || getTimeVN();
  const endVN = endTime || getTimeVN();
